...
encoded_input = Input(shape=(8,8,3))
...
model.load_weights("Models/AutoEncoder_Cifar10_Deep_weights.10-0.60-0.60.hdf5") 
# model.load_weights(saveDir + "AutoEncoder_Cifar10_Deep_weights.10-0.62-0.62.hdf5") 
...
def showOrigDec(orig, enc, dec, num=10):
    ...
    for i in range(n):
        ...
        plt.imshow(enc[i].reshape(8, 8, 3), interpolation="nearest")
        ax.get_xaxis().set_visible(False)
        ax.get_yaxis().set_visible(False)
...
normal_dists = []
for i in range(10):
    normal_dists.append(np.random.normal(0, 1, 192))
    
decoded_dists = []
for i in range(10):
    decoded_dists.append(decoder.predict(normal_dists[i].reshape(1,8,8,3)))
...
for i in range(n):
    ...
    plt.imshow(normal_dists[i].reshape(8, 8, 3), interpolation="nearest")
    ax.get_xaxis().set_visible(False)
    ax.get_yaxis().set_visible(False)
