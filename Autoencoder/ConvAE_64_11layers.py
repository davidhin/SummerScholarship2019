...
encoded_input = Input(shape=(4,4,4))
...
#                    Train Model                  #
# ----------------------------------------------- #
model.load_weights("Models/64Core11Layers.hdf5")
...
def showOrigDec(orig, enc, dec, num=10):
    ...
    for i in range(n):
        ...
        plt.imshow(enc[i].reshape(4,4,4), interpolation="nearest")
        ax.get_xaxis().set_visible(False)
        ax.get_yaxis().set_visible(False)
        ...
        ax.get_xaxis().set_visible(False)
        ax.get_yaxis().set_visible(False)
    plt.savefig("Outputs/plt_reconstructed_64_11layers.png")
    plt.show()
...
decoded_dists = []
for i in range(10):
    decoded_dists.append(decoder.predict(normal_dists[i].reshape(1,4,4,4)))
...
for i in range(n):
    ...
    plt.imshow(normal_dists[i].reshape(4,4,4), interpolation="nearest")
    ax.get_xaxis().set_visible(False)
    ax.get_yaxis().set_visible(False)
    ...
    ax.get_xaxis().set_visible(False)
    ax.get_yaxis().set_visible(False)
plt.savefig("Outputs/plt_decoded_dists_64_11layers.png")
plt.show()
